...
### 1.3 Docs home (/docs)
- [ ] Card grid: 4 cards (Getting Started, Deploy Dashboard, Enroll Node, Smoke Tests)
- [ ] Each card has colored icon background + title + 1-line description
- [ ] Clicking a card navigates to the correct docs page
- [ ] "Quick start" terminal block with 3 commands, syntax highlighted
- [ ] Stats row: 2 (stacks), 9 (surfaces), 7 (listeners), 4 (phases) in different colors

### 1.4 Docs sub-pages (/docs/getting-started, /docs/deploy-dashboard, /docs/enroll-node, /docs/smoke-tests)
...
- [ ] Shows 3 stat cards: Registered nodes, Captured sessions, Captured requests
...
### 3.2 Nodes (/nodes)
- [ ] Shows "test-node-1" card with ONLINE status badge (green, pulsing dot)
...
- [ ] Stats: node count + queue pressure
- [ ] Manual backfeed form: select node, category, subcategory, prompt
...
- [ ] Event cards: method badge, service, path, node, actor, IP, strategy, timestamp, response code, user agent
